...
def generate_report(df: pl.LazyFrame) -> pl.DataFrame:
    ...
    numeric_cols = [col for col, dtype in df.schema.items() if dtype in numeric_types]
    ...
    report = df.select(metrics).with_columns(pl.lit(datetime.utcnow()).alias("run_ts")).collect()
    return report
